...
proc is_connect_to_end_from_source {srchd endname {end_pin_type ""} {pin_name "*"}} {
	...
	set out_pins [get_pins -filter "DIRECTION==O && NAME=~${pin_name}" -of_objects ${srchd}]
...
proc plnx_gen_conf_processor {mapping kconfprefix} {
	...
	foreach cpu [concat ${aarch64list} ${armlist} ${mblist}] {
		...
		set hd [get_cell ${cpuname}]
		set ipname [get_property IP_NAME ${hd}]
		set slaves_list {"slaves_strings"}
		foreach s [split [get_property SLAVES ${hd}]] {
			if {[llength [get_cell ${s}]] == 0} {
				continue
			} else {
	...
	foreach m ${mapping} {
		...
		foreach hd ${hds} {
			...
			if { "${baseaddr_property}" != "" } {
				set uart_baseaddr [get_property ${baseaddr_property} ${hd}]
			}
...
# source hsi tools script which are not longer exported to user
set hsi_path [exec which hsi]
set hsi_root_dir [file dirname [file dirname "${hsi_path}"]]
foreach lib_file [glob -directory "${hsi_root_dir}/scripts/hsm/" xillib*.tcl ] {
	source $lib_file
}
